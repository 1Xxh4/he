...
int db_list_employees(char *result, size_t result_size) {
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        // 确保 dept 不为 NULL
        const char *dept_str = dept ? dept : "NULL";
        ...
        if (sex) {
            strncpy(sex_buf, sex, sizeof(sex_buf) - 1);
        }
        if (birth) {
            strncpy(birth_buf, birth, sizeof(birth_buf) - 1);
        }
        if (dept_str) {
            strncpy(dept_buf, dept_str, sizeof(dept_buf) - 1);
        }

        printed = snprintf(result + current_len, result_size - current_len,
                 "%-6d%-8s%-6s%-12s%-s\n",
                 serial_num, name_buf, sex_buf, birth_buf, dept_buf);
